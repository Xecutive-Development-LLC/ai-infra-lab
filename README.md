...
**Status:** End-to-end path is working — GPU passthrough, driver/CUDA, PyTorch, vLLM, and the OpenAI-compatible API have all been validated with a successful first inference request. Controlled benchmarking, concurrency testing, persistence (systemd/Docker), security, observability, model evals, and the application/agent layer are next.
...
1. Build a repeatable benchmark harness (TTFT, latency, tokens/sec, p50/p95).
2. Test concurrency (2/4/8+ requests) and observe vLLM continuous batching behavior.
3. Make CUDA PATH persistent; daemonize vLLM (systemd or Docker + NVIDIA Container Toolkit).
4. Then: observability (Prometheus/Grafana), model evals, and the first real agent/tool-calling use case.
