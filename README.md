...
**Model comparison round 2 done:** cast a wider net across vLLM's full supported-architectures list, verified against both the HuggingFace API and the live vLLM `ModelRegistry` — see `benchmarks/README.md`'s "Round 2" section and [`docs/MODEL_COMPARISON_ROUND2_RESULTS.md`](docs/MODEL_COMPARISON_ROUND2_RESULTS.md). No clean new champion, but a very promising unresolved lead: **`ibm-granite/granite-4.0-h-micro`-AWQ (3.2B params) hits 350-437 tok/s decode at 128K/91K-token prompts — more than double Qwen3.5-4B-FP8** — but the identical checkpoint is almost entirely broken at 32K context, not yet root-caused. `LiquidAI/LFM2.5-2.6B` set a new project-wide throughput record (20,997 tok/s peak) but fails 100% of requests at 128K, same failure signature as Gemma. Three other candidates (Falcon-H1-7B, Nemotron-H-8B-FP8, Ministral-3-8B) failed to load at all — a hard VRAM ceiling, a checkpoint/weight-loader incompatibility, and a `transformers`/vLLM version-skew bug, respectively, all written up as landmines. Also tested `--kv-cache-dtype fp8` (a KV-cache-only quantization, separate from every weight quantization tested so far) on the round-1 champion: **+90% KV-cache headroom, +15% decode throughput, for one flag, no new model** — the clearest "optimize at the software level" win found yet. As always: speed/capacity only, quality not evaluated — Phase E still gates any actual production switch. **Currently running: FP8 model at 128K context (`--max-model-len 131072`, no `--kv-cache-dtype` change yet)** — same live config on `192.168.60.157:8000` as before both comparison rounds; restored automatically after each.
...
6. ~~Model comparison round 2: wider vLLM-architecture sweep (Falcon-H1, Nemotron-H, Granite-4.0-H-Micro, LFM2.5, Ministral-3) + `--kv-cache-dtype fp8` VRAM lever.~~ Done — see above. AWQ/int4 finally tested (Granite-4.0-H-Micro) with a striking but unresolved result.
7. **Next session, in priority order:** (a) root-cause why Granite-4.0-H-Micro-AWQ fails at 32K but works beautifully at 128K — if fixable, it likely becomes the new champion outright; (b) adopt `--kv-cache-dtype fp8` on whichever model ends up in production, since it's a proven, low-risk win regardless of which model wins; (c) Phase E — task-specific quality/accuracy evals for the current front-runner before any actual production switch; (d) optionally chase the Gemma-4-E4B-it / LFM2.5-2.6B long-prompt failures on a future vLLM upgrade, lower priority since Qwen3.5-4B and (pending #7a) Granite-4.0-H-Micro already look stronger.
8. Make CUDA PATH persistent (still shell-session-only); daemonize vLLM (systemd or Docker + NVIDIA Container Toolkit) so it survives reboots.
9. Then: observability (Prometheus/Grafana — GPU utilization/power/VRAM/KV-cache during load is still unmeasured), and the first real agent/tool-calling use case.
10. Visual charts/graphs of the benchmark results for non-technical stakeholders — deferred until requested; raw data is all in place under `benchmarks/results/` and both `docs/MODEL_COMPARISON*_RESULTS.md` docs to build them from whenever needed.
11. Untested software-level VRAM levers worth a future pass: `--language-model-only` (skip vision/audio towers on multimodal models used text-only), `--cpu-offload-gb` (offload weights to the 62GB system RAM), `--kv-offloading-size` (offload cold KV-cache blocks to system RAM) — all confirmed to exist in the installed vLLM, none benchmarked yet.
