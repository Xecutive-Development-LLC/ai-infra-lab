...
- ~~Add authentication/API-key protection before exposing the service beyond localhost/trusted network.~~ **Built (2026-09-17), not yet deployed** — `docker-compose.yml` now reads `VLLM_API_KEY` from `.env` (`env_file:`, gitignored, vLLM reads it natively, no CLI flag change). Confirmed via vLLM's own `--help` text: only gates `/v1`, `/v2`, `/inference`; `/health` and `/metrics` stay open. Neither the Operator Portal's AI features nor the agentic-orchestration layer are live yet, so flipping this on causes no active outage today — but it's a real prerequisite: both need the same key in their own config before they can call this server at all once they do go live. See `deploy/README.md`'s "Authentication" section. Not rolled out to the live VM yet — `.env` still needs to be created there and the container restarted.
