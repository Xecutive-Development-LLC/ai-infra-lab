...
- ~~Add authentication/API-key protection before exposing the service beyond localhost/trusted network.~~ **Built (2026-09-17), not yet deployed** — `docker-compose.yml` now reads `VLLM_API_KEY` from `.env` (`env_file:`, gitignored, vLLM reads it natively, no CLI flag change). Confirmed via vLLM's own `--help` text: only gates `/v1`, `/v2`, `/inference`; `/health` and `/metrics` stay open. Real coordination cost before rollout: the Operator Portal (and later the agentic-layer project) need the same key or they 401 — see `deploy/README.md`'s "Authentication" section. Not rolled out to the live VM yet — `.env` still needs to be created there and the container restarted, coordinated with those consumers.
